...
static void __init pcm049_ehci_ohci_init(void)
{
	...
	if (auxclk) {
		clk_set_rate(auxclk, 19200000);
		clk_enable(auxclk);
		usbhs_bdata.phy_clk = auxclk;
	}
	else
...
#endif
	/*
	 * Need to reset the unused USB FS Host controller (HCOCPSYS register)
	 * or else it remains active and prevents suspend to RAM.  The FS Host
	 * interface exists on the PCM049 SoM but is unused on the 1342
	 * carrier board.
	 */
	omap_writel(0, 0x4A0A9210);
}
